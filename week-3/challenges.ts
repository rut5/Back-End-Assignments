...
// the parameter must be a number and the output must also be a number
...
const map = (numbers: number[], callback: (num: number) => number): number[] => {
    const result: number[] = [];
...
console.log(map([1, 2, 3], addTwo)); // output should be [3, 4, 5]
...
function forEach(array: any[], callback: (char: any) => void) {
...
console.log(alphabet); // output should be abcd
...
const mapWith = (numbers: number[], callback: (num: number) => number): number[] => {
    const result: number[] = [];
    forEach(numbers, (num) => {
        result.push(callback(num));
    });
